...
int check_LeaderBoard()
{
    ...
    {
        // to remove
        // new player in the leader board
        printf("New Player in check_LeaderBoard\n");
    ...
    {
        // --- to remove ---
        printf("player alread present in check_LeaderBoard\n");
    ...
    return i; // u got ith position in leader board
}

void save_leader_board(people *leader_board_array, int n)
{
    printf("save leader is called");
...
void already_present(people *arr, int i, int total_players)
{
    ...
    if (arr[i].high_score < score)
    {
        ...
        if (leader_position != 0)
        {
            ...
            getch();
            // beak the leader board
            // msg for leader with position = leader_position
        }
        arr[i].high_score = score;
    }
...
// set the leaderboard... see the role is eating everything !!!

// issue is
/*
u callled a fuction
which opnes leader board file
and make itnto an leader board arr;
now
u need to update array accordinignly
rest reseting file will take care by save_leader_board
*/